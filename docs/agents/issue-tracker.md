...
Always call the CLI as `HOME=~/.config/agent-homes/<agent> exponential …` from an agent so writes are
attributed to the agent's service account, not to a human (see the workspace `CLAUDE.md`).

These are the defaults every CLI command in this repo should target.

- **Workspace**: `clear`
- **Product**: `clear`
- **Default feature**: `cmty5pgfd0003jz04pfoyibx0` (CLEAR MCP server) — pass `--feature cmty5pgfd0003jz04pfoyibx0` when creating tickets for this repo.
